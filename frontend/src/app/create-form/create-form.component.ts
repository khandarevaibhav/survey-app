...
export class CreateFormComponent implements OnInit{
  ...
  drop(event: CdkDragDrop<any>) {
    if (event.previousContainer === event.container) {
      ...
      event.previousContainer.id === 'formElements' &&      event.container.id === 'mainForm'
    ) {
      ...
      event.previousContainer.id === 'mainForm' &&      event.container.id === 'formElements'
    ) {
  ...
  @ViewChildren(ShortAnsComponent)
  shortAnsComponent!: QueryList< ShortAnsComponent>;
  @ViewChildren(NumberComponent)
  numberComponent!:QueryList< NumberComponent>;
  ...
  // onFormSubmit(){
  //   this.shortAnsComponent.forEach((child: { getValue: () => any; }) => {console.log(child.getValue())})
  // }

  
  onFormSubmit() {
    // no if(localStorage.getItem('email')!==this.emailComponent.getValue()){
    //   alert("Please enter your Current login Email..");
    //   return;
    // }
    //root object  no
    let jsonobj: RootObject = new RootObject('', '','', []);
    let initialshort, initialnum=0;
       ...
       const key = this.mainForm[i];     
        switch (key) {       
          ...
          case 'Short Answer':  
            if(initialshort==1){
              ...
              initialshort=1;
              this.shortAnsComponent.forEach((
                child:{ getValue: ()=> any;})=>{
                let que: Questions = new Questions("", "", [])
                que.questionContent = child.getValue().question;
        ...
        case 'Number':
          if(initialnum==1){
            ...
            initialnum=1;
            this.numberComponent.forEach((
              child:{ getValue: ()=> any;})=>{
              let que: Questions = new Questions("", "", [])
              que.questionContent = child.getValue().question;
              que.answerType = 'Number';
              console.log(que)
              jsonobj.questions.push(que);}
